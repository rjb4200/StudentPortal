...
export function buildEmailHtml(title: string, bodyHtml: string, loginUrl?: string, ctaText?: string): string {
  const ctaButton = loginUrl
    ? `<div style="margin:30px 0;text-align:center;">
                <a href="${loginUrl}" style="display:inline-block;background:${EMAIL_CRIMSON};color:#ffffff;text-decoration:none;font-size:16px;font-weight:800;padding:15px 30px;border-radius:10px;box-shadow:0 4px 12px rgba(164,1,4,0.25);">${escHtml(ctaText || 'View Your Dashboard')}</a>
              </div>`
    : '';
