...
The Shift Management table SHALL provide a Cancel button on each approved schedule row, allowing the admin to cancel the shift with an optional note.
...
- **THEN** the schedule status changes to `cancelled`, `cancelled_by` is set to `admin`, the row moves from the Approved tab to the Cancelled tab, and the student receives a cancellation email
