...
- [ ] 2.1 Create `src/app/onboarding/page.tsx`: client component that tracks current step (intro → registration → legal → resources → quiz → complete), renders the corresponding step component with correct props (`studentId`, `onComplete`, `onBack`, `helpEmail`), and lifts `studentId` from registration step

## 3. Verification

- [ ] 3.1 Run `npm run build` to verify no compilation errors
- [ ] 3.2 Manually verify: visit `/onboarding`, confirm crimson branding with WFD logo appears, step through the flow, confirm footer is visible
