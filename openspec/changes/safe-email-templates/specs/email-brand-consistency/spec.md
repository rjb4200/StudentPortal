...
The system SHALL define canonical email brand color constants and a shared `buildEmailHtml()` template function in `src/lib/email.ts`. All email routes SHALL import these constants rather than hardcoding color values. The template function SHALL produce a WFD-branded HTML email with crimson header, logo, charcoal divider, branded body, and CTA button styling. The function SHALL accept an optional `ctaText` parameter to customize the CTA button label (defaulting to "View Your Dashboard").
...
#### Scenario: Custom CTA button text via ctaText parameter

- **WHEN** an email template needs CTA button text other than "View Your Dashboard"
- **THEN** it passes the desired text as the `ctaText` parameter to `buildEmailHtml()`
- **AND** the button still uses the standard WFD crimson styling
